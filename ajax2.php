...
if (isset($_POST['login'])) {
    ...
    if (!$username && !$password)
     {
        header('Location:Receptionist.php?empty');
     } 
    else 
    {
       // $password=md5($password);
        $query = "SELECT * FROM Rec WHERE username = '$username'  AND passwordd='$password'";
        $result = mysqli_query($connection, $query);
        if (mysqli_num_rows($result) == 1) {
            $user = mysqli_fetch_assoc($result);
            $_SESSION['username'] = $user['username'];
            $_SESSION['user_id'] = $user['id'];
            header('Location:index2.php?dashboard');
        } else {
            header('Location:Receptionist.php?loginE');
        }
    }
...
if (isset($_GET['delete_room'])) {
    ...
    $result = mysqli_query($connection, $sql);
    if ($result) {
        header("Location:index2.php?room_mang1&success");
    } else {
        header("Location:index2.php?room_mang1&error");
    }
}
...
if (isset($_POST['createComplaint'])) {
    ...
    $result = mysqli_query($connection, $query);
    if ($result) {
        header("Location:index2.php?complain&success");
    } else {
        header("Location:index2.php?complain&error");
    }
...
if (isset($_POST['resolve_complaint'])) {
    ...
    $result = mysqli_query($connection, $query);
    if ($result) {
        header("Location:index2.php?complain&resolveSuccess");
    } else {
        header("Location:index2.php?complain&resolveError");
    }
}
...
if (isset($_POST['change_shift'])) {
    ...
    if ($result && $insert_result && $update_result) {
        header("Location:index2.php?staff_mang&success");
    } else {
        header("Location:index2.php?staff_mang&error");
    }
}
